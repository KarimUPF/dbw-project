...
class DomainHasProtein(db.Model):
    ...
    def __repr__(self):
        return f"DomainHasProtein(Domain: {self.domain_id}, Protein: '{self.protein_accession_id}')"from app import db
...
class Organism(db.Model):
    ...
    def __repr__(self):
        return f"Organism('{self.scientific_name}', '{self.common_name}')"from app import db
...
class ProteinHasPTM(db.Model):
    ...
    def __repr__(self):
        return f"ProteinHasPTM(Protein: '{self.protein_accession_id}', PTM: {self.ptm_id}, Position: {self.position})"from app import db
...
class Protein(db.Model):
    ...
    def __repr__(self):
        return f"Protein('{self.name}', '{self.sequence}')"from app import db
...
class PTM(db.Model):
    ...
    def __repr__(self):
        return f"PTM('{self.type}')"from app import db
...
class QueryHasProtein(db.Model):
    ...
    def __repr__(self):
        return f"QueryHasProtein(Query: {self.query_id}, Protein: '{self.protein_accession_id}')"from app import db

class Query(db.Model):
    query_id = db.Column(db.Integer, primary_key=True)
    parameters = db.Column(db.String(45), nullable=True)
    summary_Table = db.Column(LargeBinary, nullable=True)
    graph = db.Column(LargeBinary, nullable=True)
